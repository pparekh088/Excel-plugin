...
- **INV-2** No silent writes. Propose → preview → approve → apply → log.
  Trusted-session mode auto-approves only plans that are entirely LOW risk, and
  a repair inherits the original approval only while it stays inside the cells
  and risk tier the user accepted — anything wider is asked again.
